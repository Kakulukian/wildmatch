...
import { wildmatch, WM_CASEFOLD, WM_PATHNAME } from "../src/index";
...
// Mirrors t3070-wildmatch.sh's `match` helper:
//   match <glob> <iglob> <pathmatch> <ipathmatch> <text> <pattern>
// - glob       => wildmatch   (WM_PATHNAME)
// - iglob      => iwildmatch  (WM_PATHNAME | WM_CASEFOLD)
// - pathmatch  => pathmatch   (0)
// - ipathmatch => ipathmatch  (WM_CASEFOLD)
function match(
  glob: 0 | 1,
  iglob: 0 | 1,
  pathmatch: 0 | 1,
...
describe("wildmatch (t3070 parity)", () => {
  it.each(cases)(
    "$label: $pattern vs $text (flags=$flags) -> $expected",
    ({ pattern, text, flags, expected }) => {
      expect(wildmatch(pattern, text, flags)).toBe(expected);
    },
  );
});
